...
COMPOSE_FILES="-f $TEMPLATES_DIR/compose.currents.yml"

# Add service-specific files based on collected services
for service in $SERVICES; do
    COMPOSE_FILES="$COMPOSE_FILES -f $TEMPLATES_DIR/compose.${service}.yml"
done
...
HEADER="# Generated by: ./scripts/generate-compose.sh
# Profile: $PROFILE_NAME - $PROFILE_DESC
"

# Generate the merged compose file with header
echo "$HEADER" > "$OUTPUT_FILE"
COMPOSE_PROJECT_NAME="currents_${PROFILE_NAME}" docker compose $COMPOSE_FILES config --no-path-resolution --no-interpolate --no-env-resolution >> "$OUTPUT_FILE"
